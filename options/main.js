...
function saveFilter() {
  ...
  for (i = 0; i < filterArray.length; i++) {
    filterTemp = filterArray[i].replace(/^\s/g, "");
...
function fill() {
  ...
  document.querySelector("#feedBox").onchange = saveFeed;

  document.querySelector("#filterBox").onchange = saveFilter;
}

document.addEventListener("DOMContentLoaded", function () {
  document.querySelector('#feedPage').style.display = "block";
  fill();
}, false);